...
public class TinyDB {
    ...
    public void putListUser(String key, ArrayList<User> playerList){
        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<>();
        for(User user: playerList){
            objStrings.add(gson.toJson(user));
        }
        putListString(key, objStrings);
    }

    //No need Class<?> mClass parameter. Because we know it is Player!
    public ArrayList<User> getListUser(String key){
        Gson gson = new Gson();
        ...
        for(String jObjString : objStrings){
            User user  = gson.fromJson(jObjString,  User.class);
            userList.add(user);
        }
        return userList;
    }
